...
library(readr)
library(data.table)
sub <- fread("suset.csv",header = TRUE)
...
```{r}
sub_update<-sub
sub_update<-sub_update[which(!is.na(sub_update$normTitle)),]
for(i in 1:nrow(sub_update)){
  if(is.na(sub_update$industry[i])){
    sub_update$industry[i]<-find_industry(industry_word,word_vector = c(sub_update$normTitle[i]))
  }
  print(i)
}

sum(is.na(sub_update$industry))
write.csv(sub_update,"subset_updated.csv")
#sub_update is sent
#sub_update2<-sub
#sub_update2<-sub_update2[which(is.na(sub_update2$normTitle)|!is.na(sub_update2$industry)),]

sub_update3<-sub
sub_update3<-sub_update3[which(is.na(sub_update3$normTitle)&(!is.na(sub_update3$industry))),]
sum(is.na(sub_update3$industry))
sub_update4<-rbind(sub_update,sub_update3)
sum(is.na(sub_update4$industry))
write.csv(sub_update4,"subset_updated4.csv")
```

```{r}
uni_sub_update<-sub_update4
for (i in 1:nrow(uni_sub_update)) {
  temp<-unlist(strsplit(uni_sub_update$industry[i],","))
  if(length(temp)>1){
    uni_sub_update[i,]$industry=temp[1]
    for(j in 2:length(temp)){
      plusrow<-uni_sub_update[i,]
      plusrow$industry=temp[j]
      uni_sub_update<-rbind(uni_sub_update,plusrow)
    }
  }
  
}
write.csv(uni_sub_update,"uni_sub_update.csv")
#ncol(uni_sub_update)
